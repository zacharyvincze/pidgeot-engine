...
void Player::onUpdate() {
    ...
    if (m_input.wasKeyHeld(SDL_SCANCODE_UP)) m_position.y -= 1;
    if (m_input.wasKeyHeld(SDL_SCANCODE_DOWN)) m_position.y += 1;
    if (m_input.wasKeyHeld(SDL_SCANCODE_LEFT)) m_position.x -= 1;
    if (m_input.wasKeyHeld(SDL_SCANCODE_RIGHT)) m_position.x += 1;
    m_boundingBox = m_position;
}
